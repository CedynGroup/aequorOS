...
|---|---|---|---|
| Risk service (FastAPI) | `apps/risk-service` | 8003 | Regulatory calc engines, DB, S3 |
| Cash-flow ML (FastAPI) | `apps/cashflow-ml` | 8010 | Trained LSTM cash-flow model |
| Product UI (Next.js) | `aequoros-demo` | 3001 | Bank Treasurer console |
...
cd "apps/risk-service" && docker compose up -d
...
#    Either: cd apps/risk-service && .venv/bin/python scripts/seed_sample_bank.py
#    Or POST /api/v1/banks/seed-demo with the demo tenant headers.

# 6. Demo UI
cd "../.." && pnpm --filter aequoros-demo dev   # http://localhost:3001
```

Demo tenant (baked into `aequoros-demo/lib/api/client.ts`, override via
...
- `apps/risk-service`: **498 passed, 16 skipped** (`ruff`, `basedpyright` clean).
- `packages/risk-service-api`: typecheck + generated-contract tests pass.
- `aequoros-demo`: `typecheck` + `build` pass (24 static routes).
- `apps/cashflow-ml`: 20 tests pass; model beats static baseline.
...
- `apps/risk-service/.env` (local, untracked) points `DATABASE_URL` at a Homebrew
