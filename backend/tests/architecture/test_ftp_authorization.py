"""FTP routes must remain on service-enforced scoped-binding boundaries."""
...
def test_ftp_run_uses_scoped_mutation_principal_without_role_fallback() -> None:
    dependencies = _dependency_names(
        _route("/api/v1/banks/{bank_id}/ftp/run-all-scenarios", "POST")
    )
...
def test_ftp_dashboard_leaves_authority_to_the_exact_service_gate() -> None:
    dependencies = _dependency_names(_route("/api/v1/banks/{bank_id}/ftp/dashboard", "GET"))
