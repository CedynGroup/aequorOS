...
export default function SdiCapitalView({
  bankId,
}: {
  bankId: string | undefined;
}) {
  const { moduleScope } = useBankContext();
  const aggregateBankId = moduleScope.capitalAggregatedView
    ? bankId
    : undefined;
  const assuranceBankId = moduleScope.capitalRestrictedView
    ? bankId
    : undefined;
  const summary = useSdiCapitalSummary(aggregateBankId);
  const assurance = useSdiCapitalAssurance(assuranceBankId);
  const checks = useSdiCapitalChecks(aggregateBankId);
  ...
  const scopeSuffix =
    chargedRiskClasses.length > 0 && excludedRiskClasses.length > 0
      ? ` · ${chargedRiskClasses
          .map((rc) =>
            (RISK_CLASS_LABELS[rc.risk_class] ?? rc.risk_class).toLowerCase(),
          )
          .join(", ")} only`
      : "";
  const capitalHeadroom =
    cap?.car_pct === null || !cap || carMin === null
      ? null
  ...
  const bucketColumns: Column<LoanGradeBucket>[] = [
    {
      key: "grade",
      header: "Grade",
      render: (b) => <span className="capitalize text-navy">{b.grade}</span>,
    },
    {
      key: "count",
      header: "Loans",
            ...
            <PageContainer className="space-y-6">
              <div className="grid gap-6 xl:grid-cols-2">
                <SectionCard
                  ...
                  subtitle="Historical points are shown only where the accepted position book and capital structure share an as-of date."
                >
                  <SdiCapitalTrendChart
                    data={assuranceHistory}
                    carMinPct={carMin}
                  />
                </SectionCard>
                <SectionCard
                  ...
                  <dl className="grid grid-cols-2 gap-x-5 gap-y-4 text-caption">
                    <div>
                      ...
                      <dd className="mt-1 text-body font-medium text-navy">
                        {assurance.data.mapped_gl_capital_ghs === null
                          ? "Not mapped"
                          : fmtCurrency(
                              num(assurance.data.mapped_gl_capital_ghs),
                            )}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd className="mt-1 text-body font-medium text-navy">
                        {assurance.data.capital_to_gl_difference_ghs === null
                          ? "Not computable"
                          : fmtCurrency(
                              num(assurance.data.capital_to_gl_difference_ghs),
                            )}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd className="mt-1 text-body font-medium text-navy">
                        {currentAssurance.actual_provision_ghs === null
                          ? "Not supplied"
                          : fmtCurrency(
                              num(currentAssurance.actual_provision_ghs),
                            )}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd className="mt-1 text-body font-medium text-navy">
                        {currentAssurance.provision_coverage_pct === null
                          ? "Not computable"
                          : `${num(currentAssurance.provision_coverage_pct).toFixed(2)}%`}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-slate">Statutory reserve movement</dt>
                      <dd className="mt-1 text-body font-medium text-navy">
                        {assurance.data.reserve_change_ghs === null
                          ? "No prior period"
                          : fmtCurrency(num(assurance.data.reserve_change_ghs))}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd className="mt-1">
                        <StatusPill
                          tone={
                            currentAssurance.assessment_status === "provisional"
                              ? "amber"
                              : currentAssurance.assessment_status ===
                                  "not_computable"
                                ? "critical"
                                : "slate"
                          }
                        >
                          {currentAssurance.assessment_status.replaceAll(
                            "_",
                            " ",
                          )}
                        </StatusPill>
              ...
              <SectionCard
                ...
                subtitle="This control remains blocked until calculation evidence and the prescribed BoG return template have been verified."
              >
                <div className="flex items-center gap-3">
                  <StatusPill tone="critical">Not filing-ready</StatusPill>
                  <span className="text-caption text-slate">
                    {assurance.data.filing_blockers.length} evidence blocker(s)
                  </span>
                </div>
                <ul className="mt-4 list-disc space-y-2 pl-5 text-caption text-slate">
                  {assurance.data.filing_blockers.map((blocker) => (
