  ...
  const columns: Column<RegulatoryPackageSummaryRead>[] = [
    {
      key: "return",
      header: "Return",
      // A rehearsal sits in this list beside real filings, under the same
      // return code. The marker travels with the code rather than living in a
      // column of its own, so no column choice can drop it.
      render: (pkg) => (
    ...
    },
    {
      key: "with",
      header: "With",
      // A return that was SENT BACK and one that was never sent both read
      // "generated" as a lifecycle status, so the status column alone made a
      // rejection invisible: an approver returned a return for correction and
      // the row looked untouched. The chain knows the difference — a round
      // above 1 means it has come back at least once.
      render: (pkg) => {
        // A package keeps its `current_stage_seq` after it leaves the chain —
        // nothing clears it — so a return that was filed AND acknowledged by
        // the regulator still named the Validator as holding it. Once a return
        // is filed or dead, no officer holds it.
        const settled = [
          "submitted",
          "acknowledged",
          "rejected",
        ...
        if (pkg.currentStageTitle == null) {
          return <span className="text-caption text-slate">Not in review</span>;
        }
        const round = pkg.workflowRound ?? 1;
        const returned = round > 1;
        return (
          <span className="inline-flex items-center gap-1.5">
            <StatusPill tone={returned ? "amber" : "pending"}>
              {pkg.currentStageTitle}
            </StatusPill>
            {returned && (
              <span className="text-micro text-slate">
                sent back · round {round}
              </span>
            )}
          </span>
        );
    ...
    {
      key: "validation",
      // "Validation / Passed" beside a return an approver had just rejected
      // read as "the approval passed". These are the MACHINE checks, and a
      // send-back does not make them fail — so the column says which checks it
      // means.
      header: "Machine checks",
      render: (pkg) =>
  ...
  return (
    ...
    >
      <div className="space-y-5">
        {/* A practice run reaches this record with a signature trail and a
            recorded submission, which is exactly what makes it readable as a
            filing. The notice, not the reader, carries that distinction. */}
