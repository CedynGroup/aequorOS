...
export default function BehavioralOverviewPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Behavioral Models" title="Behavioral Models" />

      <SdiModuleContext title="SDI ALM context">
        Deposit stability and non-maturity-deposit assumptions support
        proportionate liquidity and balance-sheet management. Forecast quality
        improves as more history is ingested.
      </SdiModuleContext>
      ...
      <PageContainer className="py-6 space-y-6">
        ...
        >
          <p className="text-body text-navy/85 leading-relaxed">
            ...
            accepted behavioral-assumptions batch with full model provenance (SR
            11-7) — the{" "}
            {scope.institutionClass === "sdi"
              ? "IRRBB, liquidity-management, and forecasting engines"
              : "IRRBB, LCR, FTP, and forecasting engines"}{" "}
            consume the accepted batch on their next recompute.
          </p>
        </SectionCard>
      </PageContainer>
    </>
    ...
    <div className="card p-5 h-full flex flex-col gap-4 hover:border-action/40 transition-colors">
      ...
      {query.isLoading ? (
        <p className="text-body text-slate">
          Loading observed deposit behavior...
        </p>
      ) : query.error ? (
        <p className="text-body text-slate">
          Open the workspace to review data readiness.
        </p>
      ) : (
        <>
          <div>
            ...
            <p className="mt-1 text-caption text-slate">
              Runoff, churn proxy, seasonality, beta, and lag
            </p>
          </div>
          <p className="text-caption text-slate leading-relaxed">
            {scenarios.length} approved CFP behavioral scenario
            {scenarios.length === 1 ? "" : "s"} linked to documented actions.
          </p>
        </>
      )}
    </div>
    ...
    <div className="card p-5 h-full flex flex-col gap-4 hover:border-action/40 transition-colors">
      ...
      {offline || query.error ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {result && (
              <StatusPill
                tone={result.method === "ml" ? "compliant" : "pending"}
              >
      ...
      <div className="mt-auto border-t border-border-light -mx-5 px-5 pt-3 flex items-center justify-between gap-2 flex-wrap">
        <span className="flex items-center gap-1.5 flex-wrap">
          <span className="text-micro uppercase tracking-wider text-slate">
            Feeds
          </span>
