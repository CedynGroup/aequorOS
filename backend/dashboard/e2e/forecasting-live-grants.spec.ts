...
test("account-only and baseline members retain disabled Forecasting workspaces", async ({
  ...
  for (const role of ["account_admin", "invite_fresh"] as const) {
    const context = await browser.newContext({
      storageState: path.join(E2E_TMP, `${role}.json`),
    });
    const page = await context.newPage();
    const requests: string[] = [];
    page.on("request", (r) => {
      if (moduleRequest.test(r.url())) requests.push(r.url());
    });
    for (const route of [
      "",
      "/scenario",
    ...
    if (evidence)
      await page.screenshot({
        path: path.join(evidence, `forecasting-${role}.png`),
        fullPage: true,
      });
    await context.close();
  }
...
test("real Forecasting grants separate summary, confidential reading, and execution", async ({
  browser,
  request,
}) => {
  test.slow();
  ...
  async function api(
    ...
    const response = await request.fetch(`${E2E_API_ORIGIN}/api/v1${route}`, {
      ...
      data,
      headers: {
        Authorization: `Bearer ${await mintBackendToken(role, role === "admin" ? E2E_USERS.admin.authv : authv)}`,
      },
    });
  ...
  async function grant(sensitivity: string, bundle = "viewer") {
    ...
    const created = await api(
      ...
      { ...data, expected_authority_sentence: preview.body.authority_sentence },
      "admin",
    );
  ...
  expect((await api("GET", `${bank}/forecast/scenarios`)).status).toBe(403);
  {
    ...
    for (const route of ["nii", "optimizer", "whatif"]) {
      ...
      await expect(page.getByRole("tooltip")).toContainText(
        "Requires Forecasting · Aggregated · View",
      );
      await expect(
    ...
    if (evidence)
      await page.screenshot({
        path: path.join(evidence, "forecasting-real-confidential-reader.png"),
        fullPage: true,
      });
    await context.close();
  }
    ...
    ).toBe(403);
  {
    ...
    await expect(page.getByRole("tooltip")).toContainText(
      "Requires Forecasting · Confidential · Run",
    );
    if (evidence)
      await page.screenshot({
        path: path.join(evidence, "forecasting-real-reader.png"),
        fullPage: true,
      });
    await context.close();
  }
  ...
  for (const [route, data] of payloads) {
    const result = await api("POST", `${bank}/${route}`, {
      reporting_period_id: period,
      ...data,
    });
    ...
    if (route === "forecast/runs") {
      ...
      expect(
        (await api("GET", `${bank}/forecast/runs/${result.body.id}`)).status,
      ).toBe(200);
      expect(
        (await api("GET", `/banks/BK-UNKNOWN1/forecast/runs/${result.body.id}`))
          .status,
      ).toBe(404);
    }
  }
  expect(
    (
      await api(
        "GET",
        `${bank}/forecast/runs/00000000-0000-4000-8000-000000000001`,
      )
    ).status,
  ).toBe(404);
  const summaryGrant = {
  ...
  page.on("request", (r) => {
    if (/\/forecast\/runs\/[^/?]+|\/reverse-stress\/latest/.test(r.url()))
      details.push(r.url());
  });
  await page.goto("/forecasting");
  await expect(
  ...
  await expect(page.getByRole("tooltip")).toContainText(
    "Requires Forecasting · Confidential · View",
  );
  expect(details).toEqual([]);
  if (evidence)
    await page.screenshot({
      path: path.join(evidence, "forecasting-real-aggregated-reader.png"),
      fullPage: true,
    });
  await context.close();
  if (evidence)
    await writeFile(
      path.join(evidence, "forecasting-live-grant-api.json"),
