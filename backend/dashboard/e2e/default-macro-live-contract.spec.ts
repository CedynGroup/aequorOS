...
test("system contracts, governed clones, and real denied authority", async ({
  request,
  page,
}) => {
  ...
  async function api(
    ...
    const response = await request.fetch(`${E2E_API_ORIGIN}/api/v1${route}`, {
      ...
      data,
      headers: {
        Authorization: `Bearer ${await mintBackendToken(role, version)}`,
      },
    });
    ...
    if (evidence)
      writeFileSync(
        path.join(evidence, "macro-live-api.json"),
  ...
  expect(
    list.body.scenarios.every(
      (s: any) =>
        s.owner === "system" && s.status === "approved" && s.is_immutable,
    ),
  ).toBe(true);
  const scenarios = Object.fromEntries(
    list.body.scenarios.map((s: any) => [s.code, s]),
  );
  const period = (await api("GET", `${bank}/reporting-periods`)).body.periods[0]
    .id;
  const run = (id: string, role = "analyst", version?: number) =>
    api(
      "POST",
      `${bank}/enterprise-stress/runs`,
      {
        scenario_id: id,
        reporting_period_id: period,
        reason: "Live default scenario verification",
      },
      role,
      version,
    );
  const calibrations: Record<string, Record<string, number>> = {
    parallel_up_200: { parallel_bp: 200, short_bp: 200, long_bp: 200 },
    parallel_down_200: { parallel_bp: -200, short_bp: -200, long_bp: -200 },
  ...
  for (const scenario of Object.values(scenarios) as any[]) {
    ...
    if (calibrations[suffix]) {
      const translation = await api(
        "GET",
        `/macro-scenarios/${scenario.id}/translation/irr`,
      );
      expect(
        Object.fromEntries(
          Object.entries(translation.body.shocks).map(([k, v]) => [
            k,
            Number(v),
          ]),
        ),
      ).toEqual(calibrations[suffix]);
    }
  ...
  for (const action of ["submit", "approve", "archive"]) {
    expect(
      (
        await api(
          "POST",
          `/macro-scenarios/${system.id}/${action}`,
          { reason: "Attempt system mutation" },
          action === "approve" ? "approver" : "analyst",
        )
      ).status,
    ).toBe(409);
  }
  expect(
    (
      await api("PATCH", `/macro-scenarios/${system.id}`, {
        name: "Changed",
        reason: "Attempt system edit",
      })
    ).status,
  ).toBe(409);
  const systemRun = await run(system.id);
  const clone = await api("POST", `/macro-scenarios/${system.id}/clone`, {
    reason: "Customize system default",
  });
  ...
  expect(
    (
      await api("PATCH", `/macro-scenarios/${id}`, {
        name: "Customized organization scenario",
        reason: "Exercise editable clone",
      })
    ).status,
  ).toBe(200);
  const collision = await api("POST", "/macro-scenarios", {
    code: system.code,
    name: "Same-code organization scenario",
    scenario_type: clone.body.scenario_type,
    severity: clone.body.severity,
    horizon_years: 3,
    paths: clone.body.paths.map(
      ({ variable, year_index, base_value, stress_value }: any) => ({
        variable,
        year_index,
        base_value,
        stress_value,
    ...
    reason: "Exercise immutable identity",
  });
  expect(collision.status).toBe(201);
  id = collision.body.id;
  expect(
    (
      await api("POST", `/macro-scenarios/${id}/submit`, {
  ...
  for (const [scenarioId, expected] of [
    [system.id, systemRun.body.run_id],
    [id, clonedRun.body.run_id],
  ]) {
    const latest = await api(
      "GET",
      `${bank}/enterprise-stress/latest?reporting_period_id=${period}&scenario_id=${scenarioId}`,
    );
    expect(latest.body.run_id).toBe(expected);
    expect(
      (await api("GET", `${bank}/enterprise-stress/runs/${expected}`)).body
        .scenario_id,
    ).toBe(scenarioId);
  }
  for (const rotation of ["steepener", "flattener"]) {
    const draft = await api(
      "POST",
      `/macro-scenarios/${scenarios[`system_irr_${rotation}`].id}/clone`,
      { reason: "Customize long end alone" },
    );
    expect(draft.status).toBe(201);
    const draftId = draft.body.id;
    const paths = draft.body.paths.map(
      ({ variable, year_index, base_value, stress_value }: any) => ({
        variable,
        year_index,
        base_value,
        stress_value: variable === "policy_rate" ? base_value : stress_value,
      }),
    );
    expect(
      (
        await api("PATCH", `/macro-scenarios/${draftId}`, {
          paths,
          reason: "Hold policy rates flat",
        })
      ).status,
    ).toBe(200);
    expect(
      (
        await api("POST", `/macro-scenarios/${draftId}/submit`, {
    ...
    expect(
      (
        await api(
          "POST",
          `/macro-scenarios/${draftId}/approve`,
          { reason: "Independent review" },
          "approver",
        )
      ).status,
