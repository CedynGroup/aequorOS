...
test.describe('attestation surfaces', () => {
  ...
  test('a generated return shows its attestation state as unsigned', async ({ page }) => {
    ...
    // from an absence.
    await expect(page.getByText('Attestation', { exact: false }).first()).toBeVisible();
    await expect(page.getByText(/unsigned|not certified|awaiting/i).first()).toBeVisible();
  });
});
...
test.describe('the certification ceremony', () => {
  ...
  test('opting in locks submission, and the ceremony enforces what it shows', async ({
    ...
    try {
      const opted = await page.request.put(`${API}/attestation/signing-policies`, {
        headers: authorize,
        data: { ...scope, require_signature: true, reason: 'e2e: drive the ceremony' },
      });
      ...
      await expect(page.getByText('Cleared to submit', { exact: true })).toHaveCount(0);
      ...
      await expect(page.getByText(/section 93\(3\)/i)).toBeVisible({ timeout: 20_000 });
      await expect(page.getByText(/complete and accurate/i).first()).toBeVisible();
      ...
      await placeBothSignatureFields(page, page.getByTestId('signing-workspace'));
      ...
      await expect(page.getByText(/not configured for this institution/i)).toBeVisible();
      ...
      const reopened = await page.request.put(`${API}/attestation/signing-policies`, {
        headers: authorize,
        data: {
          ...scope,
...
test.describe('SSO step-up route guards', () => {
  ...
  test('the step-up start requires the package it is signing for', async ({ page }) => {
    const response = await page.request.get(
  ...
  test('certify cannot be driven without a held authorisation', async ({ page }) => {
...
test.describe('the signing workspace', () => {
  ...
  test('places typed fields from the palette, and refuses an illegible box', async ({
    ...
    try {
      const opted = await page.request.put(`${API}/attestation/signing-policies`, {
        headers: authorize,
        data: { ...scope, require_signature: true, reason: 'e2e: open the workspace' },
      });
      expect(opted.ok()).toBeTruthy();
      ...
      await expect(workspace.getByText(/section 93\(3\)/i)).toBeVisible({ timeout: 30_000 });
      await expect(workspace.getByText(/[0-9a-f]{4}…[0-9a-f]{4}/).first()).toBeVisible();
      ...
      await expect(workspace.getByText(/is below the .* pt minimum/i)).toHaveCount(0);
      ...
      await expect(
        workspace.locator('[data-signing-role="approver"][data-field-type="signature"]'),
      ).toBeVisible();
      ...
      await expect(dateField).toContainText(new Date().toISOString().slice(0, 10));
      ...
      await page.mouse.move(start.x + start.width / 2 + 40, start.y + start.height / 2 + 60);
      await page.mouse.up();
      ...
      await page.mouse.down();
      await page.mouse.move(grown.x + grown.width - 60, grown.y + grown.height - 20);
      await page.mouse.move(grown.x + 4, grown.y + 4);
      await page.mouse.up();

      await expect(workspace.getByText(/below the .* pt minimum/i).first()).toBeVisible({
        timeout: 10_000,
      });
      ...
      await expect(
        workspace.getByRole('button', { name: /Certify and send|Certify and freeze/ }),
      ).toBeDisabled();
    } finally {
      // Hand the gate back however this ends, or every other submission
      // journey in the run inherits a locked return.
...
test.describe('the filed document', () => {
  ...
  test('an unsigned return offers the base export and claims no signature', async ({
    ...
    const latest = versions.find((version: { is_latest: boolean }) => version.is_latest);
