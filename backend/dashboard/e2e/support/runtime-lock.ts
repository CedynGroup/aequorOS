...
function readOwner(lockPath: string): LockOwner | undefined {
  try {
    return JSON.parse(readFileSync(path.join(lockPath, OWNER_FILE), 'utf8')) as LockOwner;
  } catch {
    return undefined;
  }
}
...
export function acquireE2ERunLock(tmpPath: string): void {
  ...
  while (true) {
    try {
      ...
      const ownerDetail = currentOwner ? ` (owner PID ${currentOwner.pid})` : '';
      throw new Error(
  ...
  try {
    writeFileSync(path.join(lockPath, OWNER_FILE), `${JSON.stringify(owner)}\n`, {
      flag: 'wx',
    });
  } catch (error) {
    rmdirSync(lockPath);
