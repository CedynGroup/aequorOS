...
test("request expiry and route guards reject malformed authority; equivalent requests resolve once", async ({
  request,
}) => {
  const memberHeaders = {
    Authorization: `Bearer ${await mintBackendToken("access_request_member")}`,
  };
  const ownerHeaders = {
    Authorization: `Bearer ${await mintBackendToken("admin")}`,
  };
  const payload = {
    route: "/liquidity/forecast",
  ...
  for (const [patch, status] of [
    ...
    const response = await request.post(
      `${API}/authorization/access-requests`,
      { headers: memberHeaders, data: { ...payload, ...patch } },
    );
  ...
  for (const route of ["/liquidity/forecast", "/liquidity/monitoring"]) {
    const response = await request.post(
      `${API}/authorization/access-requests`,
      { headers: memberHeaders, data: { ...payload, route } },
    );
    expect(response.status()).toBe(201);
    ids.push((await response.json()).id);
  }
  const grant = {
    principal_user_id: E2E_USERS.access_request_member.id,
    role_bundle: "viewer",
  ...
  expect(
    (await pending.json()).requests.filter((r: { id: string }) =>
      ids.includes(r.id),
    ),
  ).toEqual([]);
  if (evidence)
...
test("Settings loads institutions and grant reason picker requires expiry", async ({
  ...
  await expect(
    page.getByText("Sample Bank Ltd", { exact: true }).first(),
  ).toBeVisible();
  if (evidence)
    await page.screenshot({
      path: path.join(evidence, "owner-settings-institution.png"),
      fullPage: true,
    });
  ...
  await expect(
    page.getByRole("heading", { name: "ICAAP review chain" }),
  ).toBeVisible();
  await expect(
    page.getByRole("navigation", { name: /breadcrumb/i }),
  ).toHaveCount(0);
  if (evidence)
    await page.screenshot({
      path: path.join(evidence, "icaap-workflow-no-breadcrumb.png"),
      fullPage: true,
    });
  const pending = await request.post(`${API}/authorization/access-requests`, {
    headers: {
      Authorization: `Bearer ${await mintBackendToken("access_extra_member")}`,
    },
    data: {
      route: "/fx",
      institution_id: "BK-SAMP0001",
  ...
  await expect(
    dialog.getByLabel("Reason category").locator("option"),
  ).toHaveCount(7);
  ...
  await dialog
    ...
    .selectOption("incident_break_glass");
  await expect(dialog.locator('input[type="datetime-local"]')).not.toHaveValue(
    "",
  );
  if (evidence)
    await page.screenshot({
      path: path.join(evidence, "grant-reason-expiry.png"),
      fullPage: true,
    });
  await dialog.getByRole("button", { name: "Review grant" }).click();
  await dialog.getByRole("button", { name: "Grant access" }).click();
  await expect(
    dialog.getByText("Grant created", { exact: true }),
  ).toBeVisible();
  if (evidence)
    await page.screenshot({
      path: path.join(evidence, "additional-grant-after-request.png"),
      fullPage: true,
    });
  await context.close();
});
