...
function isIPv6Literal(host: string): boolean {
  ...
  if (compressedAt !== -1 && compressedAt !== address.lastIndexOf("::"))
    return false;
...
const DEV_LOOPBACK_HOSTS: ReadonlySet<string> = new Set([
  "127.0.0.1",
  "localhost",
  "::1",
]);
...
export class OutboundTargetBlocked extends Error {
  ...
  constructor(field: string, reason: string, internalDetail: string) {
    super(
      `${field} is not a permitted destination for an outbound connection.`,
    );
...
const METADATA_BYTES: readonly (readonly number[])[] = METADATA_ADDRESSES.map(
  (address) => {
...
function embeddedIPv4(
  ...
  if (b[0] === 0x20 && b[1] === 0x01 && b[2] === 0x00 && b[3] === 0x00) {
    return {
      kind: "teredo",
      bytes: b.slice(12, 16).map((value) => value ^ 0xff),
    };
  }
  return null;
}
...
function classifyIPv6(b: readonly number[]): string | null {
  ...
  if (b[0] === 0x01 && b[1] === 0x00 && allZero(b.slice(2, 8)))
    return "non_routable"; // 100::/64
  if (b[0] === 0x20 && b[1] === 0x01 && b[2] === 0x0d && b[3] === 0xb8)
    return "non_routable"; // 2001:db8::/32
...
export function classifyAddress(value: string): string | null {
  ...
  if (METADATA_BYTES.some((metadata) => sameBytes(metadata, bytes)))
    return "cloud_metadata";
  const embedded = bytes.length === 16 ? embeddedIPv4(bytes) : null;
  if (
    embedded &&
    METADATA_BYTES.some((metadata) => sameBytes(metadata, embedded.bytes))
  ) {
    return "cloud_metadata";
  }
...
function loopbackDevTargetAllowed(url: URL): boolean {
  ...
  return (
    url.protocol === "http:" &&
    DEV_LOOPBACK_HOSTS.has(normalizeHost(url.hostname))
  );
}
...
const defaultResolveHost: HostResolver = async (host) => {
  ...
  const { lookup } = await import(
    /* webpackIgnore: true */ "node:dns/promises"
  );
...
export async function checkOutboundUrl(
  ...
  const allowed = (options.allowedSchemes ?? DEFAULT_ALLOWED_SCHEMES).map(
  ...
  if (!host)
    throw new OutboundTargetBlocked(field, "no_host", "URL has no host");
  if (
  ...
  if (ipVersion(host) !== 0) {
    ...
    if (reason)
      throw new OutboundTargetBlocked(field, reason, `${host} is ${reason}`);
  ...
  for (const address of addresses) {
    ...
    if (reason) {
      throw new OutboundTargetBlocked(
        field,
        reason,
        `${host} resolved to ${address} (${reason})`,
      );
    }
  }
