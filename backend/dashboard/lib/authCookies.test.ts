...
async function checkAuthResponses() {
  const request = (action: string, method = "GET") =>
    new Request(`http://127.0.0.1:3001/api/auth/${action}`, {
      method,
  ...
  for (const response of [
    ...
    const cleaned = await cleanAuthResponseCookies(
      request("signout", "POST"),
      response,
    );
    assert.equal(cleaned, response);
    assert.deepEqual(deleted(cleaned).sort(), names.slice().sort());
  }
  const emptySession = await cleanAuthResponseCookies(
    request("session"),
    Response.json(null),
  );
  ...
  for (const action of ["session", "callback/credentials"]) {
    ...
    response.headers.append(
      "set-cookie",
      "authjs.session-token.0=new-; Path=/; HttpOnly",
    );
    response.headers.append(
      "set-cookie",
      "authjs.session-token.1=session; Path=/; HttpOnly",
    );
    response.headers.append(
      "set-cookie",
    ...
    renewalRequest.headers.set(
      "cookie",
      cookieHeader +
        "; authjs.session-token.0=old-; authjs.session-token.1=session; authjs.session-token.2=tail",
    );
    await cleanAuthResponseCookies(renewalRequest, response);
    assert.deepEqual(
      deleted(response).sort(),
      [...names, "authjs.session-token.2"].sort(),
    );
    ...
    assert.ok(
      response.headers
        .getSetCookie()
        .includes("authjs.csrf-token=csrf; Path=/; HttpOnly"),
    );
  ...
  for (const [action, method, response] of [
    ...
    [
      "signout",
      "POST",
      Response.json({ url: "/api/auth/signin?error=MissingCSRF" }),
    ],
    [
      "session",
      "GET",
...
void checkAuthResponses().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
