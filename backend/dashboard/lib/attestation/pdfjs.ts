 ...
 *
 * Four decisions here are deliberate rather than incidental:
 *
 ...
 *    `workerPort` built with `new Worker(new URL(…), {type:'module'})`
 *    rather than as a `workerSrc` path, so the bundler compiles it as a module.
 *    This also preserves the workaround for the former Webpack build, whose
 *    script-mode minifier rejected the worker's ES module syntax when it was
 *    emitted as a raw asset.
