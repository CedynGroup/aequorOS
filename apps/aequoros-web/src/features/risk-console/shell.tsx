...
import { Button, Input, Select, SelectItem, Tooltip } from "../../components/ui";
...
export function Sidebar({
  ...
  const items: Array<{ tab: ConsoleTab; label: string; icon: typeof LayoutDashboard }> = [
    ...
    <aside className="sticky top-0 hidden h-screen w-56 shrink-0 border-r border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-3 lg:block">
      <div className="mb-5 flex items-center gap-2 px-1">
        ...
        <div>
          ...
          <div className="text-xs text-[rgb(var(--muted-foreground))]">Risk Console</div>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                activeTab === item.tab && "bg-[rgb(var(--muted))] text-[rgb(var(--foreground))]",
              )}
            >
...
export function TopBar({
  ...
  return (
    <div className="sticky top-0 z-20 flex h-14 items-center gap-2 border-b border-[rgb(var(--border))] bg-[rgb(var(--surface))] px-3">
      <div className="hidden text-sm font-semibold md:block">Risk operations</div>
      <div className="ml-auto flex min-w-0 items-center gap-2">
        <Input className="hidden w-72 md:block" value={orgId} onChange={(event) => setOrgId(event.target.value)} aria-label="Tenant org id" />
        <Input className="hidden w-72 xl:block" value={userId} onChange={(event) => setUserId(event.target.value)} aria-label="User id" />
        <Select
