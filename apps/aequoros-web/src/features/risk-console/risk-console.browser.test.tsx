...
describe("risk console browser interactions", () => {
  ...
  it("selects the current case through the Radix select portal", async () => {
    ...
    render(
      <TopBar
        orgId={DEFAULT_ORG_ID}
        userId={DEFAULT_USER_ID}
        setOrgId={vi.fn<TopBarProps["setOrgId"]>()}
        setUserId={vi.fn<TopBarProps["setUserId"]>()}
        cases={cases}
        caseId={cases[0].id}
    ...
    await act(async () => {
      await userEvent.click(screen.getByRole("combobox", { name: "Current case" }));
      await userEvent.click(screen.getByRole("option", { name: cases[1].title }));
    });
  ...
  it("opens the bulk action dialog in a real browser DOM", async () => {
    renderWithQuery(<BulkActionDialog selectedIds={[cases[0].id]} tenant={tenant} />);
