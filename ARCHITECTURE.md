...
| Workflow                             | Jobs                                                                                                                                                                                                                                                              |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.github/workflows/risk-service.yml` | `static` · `architecture` · `postgres` · `postgres-suite` (the only full backend suite in CI, with OpenBao) · `storage` · `journeys` (Playwright + MinIO, dispatched manually via `dashboard-journeys.yml`; quarantine pinned empty) · `api-fresh` · `real-data` (conditional) |
