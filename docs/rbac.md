...
> **As-built foundation (updated 2026-09-16):** The additive authorization slices
...
> binding matches or evaluation fails.
...
>
> **Permission-only disabled controls:** When a Liquidity or IRRBB control is unavailable
...
| Authorization foundation             | **BUILT.** The [as-built foundation](../backend/docs/authorization_foundation.md) owns binding semantics, effective-authority projection, grant administration, and the current product rollout boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `app/core/authorization.py`, `app/services/authorization.py`, `app/services/grant_administration.py`                                                                                                                                                      |
...
| Connection health (bank-IT surface)  | **BUILT** (read-only): Data Engine → Overview aggregates every configured source connection (DB-direct, T24, market-data) with live status, last sync, credential expiry, and plain-language remediation hints                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `dashboard/components/data-engine/ConnectionHealthPanel.tsx`                                                                                                                                                                                              |
| Legacy roles                         | The operational rank remains `admin > approver > analyst > examiner > viewer` for compatibility, but migration `202608280046` leaves no persisted `admin`: former admins become `account_admin`, which is outside the ladder.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `app/core/security.py:ROLES`, `app/models/user.py:USER_ROLES`                                                                                                                                                                                             |
| Endpoint enforcement                 | Liquidity and Capital are enforced through named route dependencies by one complete active binding for the exact institution or explicit organization-wide coverage. The [Capital rollout](../backend/docs/capital_enforcement_rollout.md) owns its action and sensitivity matrix, including independent CAP run plus LIQ confidential view for ILAAP refresh. The [Liquidity rollout](../backend/docs/liquidity_enforcement_rollout.md) owns the issue #165 surface, permission, and filtering matrix. Coarse legacy dependencies still enforce viewer/analyst mutation separation and selected approver gates elsewhere. SSO connection read/write, SSO request list/reject, and all integration-key lifecycle operations require organization-wide ACCOUNT/restricted `administer`; organization-directory reads separately require ACCOUNT/restricted `view`. Every API Push route requires an exact machine DATA/restricted `ingest` binding for the requested bank.                                                                                                                                                                                                                                                                                                                                                                                | `app/api/deps.py`                                                                                                                                                                                                                                         |
| Tenancy                              | Postgres RLS forced on `app.organization_id`; cross-tenant work runs on the BYPASSRLS `WORKER_DATABASE_URL` role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `app/db/session.py`, CLAUDE.md                                                                                                                                                                                                                            |
| Maker-checker                        | **Regulatory reporting already has it**: `draft→generated→validated→pending_approval→approved→submitted→acknowledged→…` with an append-only approval trail where **checker ≠ maker is enforced in the service**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `app/models/regulatory_reporting.py` (`PACKAGE_STATUSES`, `APPROVAL_ACTIONS`, `RegulatoryPackageApproval`)                                                                                                                                                |
| Authorization foundation             | **BUILT; Liquidity and Capital enforcing.** Deny-by-default evaluation over indivisible `authorization_bindings`; explicit organization-or-exact-institution resource targets; exact module, sensitivity, lifecycle, principal-type, and runtime-condition matching. Org Owner-gated create/list/revoke and tenant Members aggregation are live, with assignment-time SoD, complete grant/revoke audit evidence, and transactional `authv`/refresh-family invalidation. `/auth/me` and `/auth/effective-authority` project evaluator-derived organization and institution capabilities; `/banks` and its detail/period/fact routes require projected institution coverage. Capital adds exact aggregated/confidential/restricted reads, maker/checker actions, hidden unauthorized run objects, and dual CAP+LIQ ILAAP authorization. The [Liquidity rollout contract](../backend/docs/liquidity_enforcement_rollout.md) owns its enforced surface matrix. Other product routes retain their existing behavior until their cutover.                                                                                                                                                                                                                                                                                                                      | `app/core/authorization.py`, `app/services/authorization.py`, `app/services/grant_administration.py`, `app/features/manage_authorization.py`, `dashboard/components/settings/MembersPanel.tsx`, `backend/docs/authorization_foundation.md`                |
| Token claims                         | App access and refresh tokens carry `sub`, `org`, legacy `roles[]`, and authoritative `authv`, plus `email`/`name` when present; refresh tokens also require `jti`. Pre-`202608250044` and stale-version sessions fail closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `app/core/security.py:create_token`, `app/api/deps.py:validate_tenant_context`                                                                                                                                                                            |
| Identity in UI                       | Header + settings read the real session (name/role); route gate redirects unauthenticated → `/login`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `dashboard/components/shell/Header.tsx`, `dashboard/middleware.ts`                                                                                                                                                                                        |
>>>>>>> c3e2db6e (no-mistakes(document): Correct stale activation documentation and normalize formatting)

### The gap this spec closes

1. Selected `approver` gates remain coarse legacy-rank checks rather than exact
   module/resource binding decisions. Integration-key lifecycle and push
   authorization are binding-enforced; other product cutovers remain sequenced.
...
4. Endpoint enforcement still uses **one flat legacy role per user** outside
   the [migrated product surfaces](../backend/docs/authorization_foundation.md#product-rollout-boundary). Exact
...
5. Assignment-time scoped-grant SoD now returns authoritative allow/warn/block
   decisions. Action-time maker-checker remains workflow-specific; Capital's
   approval condition is documented in the
   [Capital rollout](../backend/docs/capital_enforcement_rollout.md).
...
The [product rollout contracts](../backend/docs/authorization_foundation.md#product-rollout-boundary)
own each migrated surface’s enforcement and access matrix. For each later migrated
vertical, construct the same explicit locator and call the service evaluator;
do not resolve authority from route names, HTTP verbs, or UI state. Keep
...
**`authorization_bindings`** _(BUILT; cutovers tracked in the [foundation rollout boundary](../backend/docs/authorization_foundation.md#product-rollout-boundary))_: one indivisible row holds
...
# BUILT (Phase-1 form, organization-wide ACCOUNT/restricted authority):
...
#   GET  /auth/sso/connection     administer — secret returned only as client_secret_set
#   PUT  /auth/sso/connection     administer — upsert; client_secret write-only
#   GET  /auth/sso/access-requests administer — list deactivated JIT stubs
#   POST /auth/sso/access-requests/{user_id}/approve
#                                 Org Owner — atomic activation (foundation contract §Scoped grant administration)
#   POST /auth/sso/access-requests/{user_id}/reject
#                                 administer — reject the never-activated stub
