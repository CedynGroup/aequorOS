...
> version invalidation. Liquidity Monitoring and Capital are enforcing product
> surfaces: each exact action and sensitivity requires an active complete
> binding, with no scalar-role fallback. Capital-plan approval preserves
> maker-checker, and ILAAP refresh requires independent CAP/run and
> LIQ/confidential-view decisions before storage.
...
| Authorization foundation             | **BUILT; Liquidity Monitoring and Capital enforcing.** Deny-by-default evaluation over indivisible `authorization_bindings`; explicit organization-or-exact-institution resource targets; exact module, sensitivity, lifecycle, principal-type, and runtime-condition matching. Org Owner-gated create/list/revoke and tenant Members aggregation are live, with assignment-time SoD, complete grant/revoke audit evidence, and transactional `authv`/refresh-family invalidation. `/auth/me` and `/auth/effective-authority` project evaluator-derived organization and institution capabilities; `/banks` and its detail/period/fact routes require projected institution coverage. Capital adds exact aggregated/confidential/restricted reads, maker/checker actions, hidden unauthorized run objects, and dual CAP+LIQ ILAAP authorization. Other product routes retain their existing behavior until their cutover.                                                                                                                                                                                                                                                                                                                                               | `app/core/authorization.py`, `app/services/authorization.py`, `app/services/grant_administration.py`, `app/features/manage_authorization.py`, `dashboard/components/settings/MembersPanel.tsx`, `backend/docs/authorization_foundation.md`                |
...
**`authorization_bindings`** _(BUILT; Liquidity Monitoring enforcing)_: one indivisible row holds
