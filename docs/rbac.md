...
> **As-built foundation (updated 2026-09-04):** The additive authorization slices
...
> version invalidation. Liquidity Monitoring is the first enforcing product
> route: exact active LIQ/confidential view authority is required, with no
> scalar-role fallback.
...
4. Endpoint enforcement still uses **one flat legacy role per user** outside
   Liquidity Monitoring. Exact institution/module/sensitivity bindings now
   enforce that first route and have an Org Owner administration surface;
   desk/currency scope is not represented yet.
...
Liquidity Monitoring is the first enforcing path: it constructs the canonical
institution target, emits `authz.binding_decision`, and denies unless one
complete active binding grants `view` on LIQ/confidential. It also denies on
evaluator failure and never falls back to `roles[]`. For each later migrated
