...
**Status: DESIGN, NOT LEGAL ADVICE.** This document is written to _satisfy_ the
...
Protection Act, 2012 (Act 843). Nothing here is a representation that it _does_
...
**Stage 1 — canonical facts** _(Class A/C only)_. Ingested data is normalised
...
**Stage 2 — the engine run** _(Class A/C only)_. `POST /banks/{bank_id}/regulatory-runs`
...
**Caveat.** Immutability is a property of _the code that exists_, not of the
schema — there are no triggers and no revoked privileges (see 1.5). And no test
pins a _literal_ golden hash, so a future change to the canonicalisation recipe
...
| `acknowledged`          | _(terminal — correction requires a granted resubmission)_ |
...
stub and returns 403 until an account administrator approves with an explicit
non-owner role.
...
Time: every timestamp is `utc_now()` — the _application host's_ wall clock.
...
| **G9**  | **A `consolidated` package renders a PDF that says "Solo"** — `basis` is stamped into the snapshot but templates hard-code the solo string, and no generator branches on basis                                                                              | `templates.py:40, 98`; `generation.py:116`                       | High — _signing a document that misstates its own basis is a defect in the instrument_ |
...
| **G13** | `snapshot_sha256` embeds `metadata.generated_at`, so it seals _a version_, not _content identity_ — two regenerations of identical data differ                                                                                                              | `generation.py:365`                                              | Medium (design must not assume otherwise)                                              |
...
`bank_products`, `bank_licenses`, `bank_name_history`) is _live, mutable
master data_ with reason-required audited mutations — it has no immutable-run
...
- Rotating `SIGNER_ID_PEPPER` changes what _new_ users would derive; it does
...
| Past signatures           | **Remain valid and attributable.** The signature record holds a denormalised `signer_id`, the signer's display name _as it stood at signing_, the certificate chain as signed, and the RFC 3161 token. Verification needs none of the live user row. |
| Their signing certificate | Revoked (§3.4). Because every signature embeds a trusted timestamp and its validation material, revocation invalidates _future_ use, not past signatures — this is precisely why the TSA is not optional.                                            |
...
843, the opaque ID keeps the signature attributable to _a determinate person_
...
mutating one. So a digest over the snapshot proves _"these are the exact figures
and fields I signed"_ for all thirteen returns.
...
unchanged but _correct for that data_. Master-data packs cannot have that today
...
This binds _which register state_ produced the pack, so a later mutation of a
...
**Class C (daily).** Binding is identical to Class A; only the _ceremony_ is in
question (G17 / C3). Nothing about the digest changes.

The per-signature payload adds who, in what capacity, and _what they were
shown_:
...
An earlier revision assembled the certificate locally and signed it _through_
...
_constrain_ it: a role that has not been granted the pattern refuses the request
...
issuance. It runs _before_ the `signer_keys` row is touched and its failure
...
a path an operator populates — _not_ from a startup fetch, so verification is
...
2. **Tamper between signatures** — pyHanko's **diff analysis** compares each
   ...
   page — is reported as tampering with the specific offending object. _This is
...
- The engine runs are already immutable; the freeze binds _which_ runs are in
...
The approver's UI states this explicitly: _"You are certifying the identical
figures the preparer certified — digest `a4f2…9c1b`, verified."_
...
- **preparer ≠ approver** — compared on `signer_user_id` _and_ on `signer_id`
...
The resolution result is subject to one deployment-wide switch:
`ATTESTATION_ESIGN_REQUIRED=0` presents every resolved policy as
signature-optional (`source="esign_disabled"`), configured mandatory rows
included. Rows are dormant, not deleted — re-enabling the flag restores them
unchanged (§9 D16).
...
| **Created using means under the signatory's sole control**                    | Non-exportable HSM key; use gated by a single-use authorisation minted only after step-up re-authentication bound to that exact package and digest; no background path can invoke a key      | **The central legal question — L1.** A server-side HSM key is _not_ physically in the signatory's possession |
...
- **Only digests leave the perimeter.** An RFC 3161 TSA receives a _hash_, never
...
- **No customer data in signature or audit records.** Signature records hold
  institution, return, period, digests, and signer identity. Return _content_
...
- **Act 930 s.93(3)** — already cited in the platform's attestation wording
  ...
  and signed as read: it exposes both the institution _and_ responsible key
...
- **Retention** — tied to the regulatory retention period, not indefinite by
  ...
  no lifecycle expiry.
- **Erasure vs. permanent attribution** — the tension is resolved _by design_:
...
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **0 — Evidential foundation** _(prerequisite; no signing yet)_ | DB-enforced append-only + hash chains; artifact versioning with `version_id`; `snapshot_sha256` verification; **`content_digest` (volatile metadata excluded)**; **`register_state_digest` for master-data packs**; TSA integration; pin reportlab; fix the consolidated-basis defect; fix `create_user.py` | G1, G2, G3, G4, G8, G9, G12, G13, G15, G16 |
...
| **L12** | Retention period for signed returns and attestation records, and whether the signature must remain _verifiable_ (not merely stored) for that whole period.                                                                                                                                                  | Drives the PAdES profile choice (B-LTA already assumes long-term verifiability) and re-timestamping policy.                                                                                                                  |
...
| **L16** | **Does an attestation over master-data packs (the five `LRT-*` corporate returns) carry the same weight as one over engine-backed returns?** Class A signatures bind a _reproducible_ `input_hash`; Class B can bind only _content_ plus a register-state digest — state, not derivation (§1.7, §3.1, G16). | If equivalence is required, the corporate register needs an immutable snapshot-run concept of its own before those packs may be signed — a material build, not a configuration change.                                       |
...
| D3  | Submission blocked until fully certified, by default                                                                                           | **Reversed since build: signature-REQUIRED by default** (2026-07-25, founder's call — `default_policy` returns `require_signature=True, require_signed_pdf=True`, preparer+approver). The original build shipped signature-OPTIONAL, activating only when an administrator configured a `return_signing_policy`; that survives as the per-return relaxation path, plus a deployment-wide kill-switch (D16)                                                                                                                                                                                                                                                                                                                                                                                                                                                | The optional default conflated what the regulator demands _of the artifact_ (C1–C4, unconfirmed) with what the institution demands _of itself_ before filing. Only the former is BoG's to answer. A bank that finds the requirement unworkable relaxes it per return in Settings — an explicit, audited act — rather than the platform waiving it silently. The pre-existing `generated_by ≠ approver` control still applies to every return                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
3. **The PKCS#11 signing path is not exercised.** `python-pkcs11` is an opt-in
   ...
   This is no longer the _production_ story, though. **OpenBao Transit
   ...
   against one. Whether an _internal_ PKI suffices, or an accredited provider is
...
7. **The browser journey covers the ceremony surfaces, not the whole
   lifecycle.** The hermetic stack now enrols disposable self-signed _software_
