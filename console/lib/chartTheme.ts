 ...
 * render time, so charts follow the console's dark tokens without JS theme
 * plumbing.
 *
 * DELIBERATE PORT of backend/dashboard/lib/chartTheme.ts (the console shares the
 * dashboard's exact token layer). SVG accepts `var(--chart-N)` strings for
 * stroke/fill, and inline tooltip styles resolve `rgb(var(--token))`
 * expressions, so everything here is a plain CSS string. Axis tick / legend
 * typography is themed centrally by the `.recharts-*` overrides in
 * app/globals.css.
 *
 * Usage:
...
/** Themed tooltip: dark raised card. */
