...
const bankColumns: Column<TenantConfigBank>[] = [
  ...
  },
  {
    ...
    header: "Jurisdiction",
    render: (b) => (
      <span className="font-mono text-caption">{b.jurisdiction_code}</span>
    ),
  },
  {
    key: "currency",
  ...
  },
  {
    ...
    header: "License",
    render: (b) => (
      <span className="text-caption text-navy/90">{b.license_type}</span>
    ),
  },
];
...
const liquidityColumns: Column<TenantLiquidityThreshold>[] = [
  ...
  },
  {
    ...
    header: "Class",
    render: (t) => (
      <span className="text-caption text-navy/90">{t.institution_class}</span>
    ),
  },
  {
    key: "pct",
    ...
    align: "right",
    render: (t) => (
      <span className="font-mono text-caption">{pct(t.threshold_pct)}</span>
    ),
  },
  {
    key: "from",
...
const capitalColumns: Column<TenantCapitalThreshold>[] = [
  ...
  },
  {
    ...
    align: "right",
    render: (t) => (
      <span className="font-mono text-caption">{pct(t.value_pct)}</span>
    ),
  },
  {
    key: "from",
  ...
  },
  {
    ...
    align: "right",
    render: (t) => (
      <span className="text-caption text-slate">{t.approved_by || DASH}</span>
    ),
  },
];
...
const keyColumns: Column<TenantIntegrationKey>[] = [
  ...
  },
  {
    ...
    header: "Key",
    render: (k) => (
      <span className="font-mono text-caption text-slate">{k.key_prefix}…</span>
    ),
  },
  {
    key: "bank",
...
export function TenantConfigSection({
  ...
  const mappingColumns: Column<TenantMappingConfig>[] = [
    ...
    },
    {
      ...
      render: (m) => (
        <span className="font-mono text-caption">v{m.version}</span>
      ),
    },
    {
      key: "status",
    ...
    >
      ...
      >
        <div>
          <Block title={`Banks (${banks.length})`}>
            {banks.length > 0 ? (
              ...
              <p className="px-4 py-3 text-caption text-slate">
                No banks provisioned.
              </p>
            )}
          </Block>

          <Block
            title={`Active mappings (${mappings.length})`}
            hint="Open a row to view the raw mapping config."
          >
            {mappings.length > 0 ? (
              <DataTable
          ...
          <Block
            title="Threshold register"
            hint="Current Board-approved internal thresholds (effective, open-ended)."
          >
            <div className="grid gap-4 px-4 py-3 lg:grid-cols-2">
              <div>
                <div className="mb-1 text-micro uppercase tracking-wider text-slate">
                  Liquidity
                </div>
                {liquidity.length > 0 ? (
                  <DataTable
      ...
      <Drawer
        ...
        description={
          mapping
            ? `${mapping.source_system} · ${mapping.source_ref} · v${mapping.version}`
            : undefined
        }
        width="w-[560px]"
      >
